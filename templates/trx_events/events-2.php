...
if ( !function_exists( 'yogastudio_template_events_2_theme_setup' ) ) {
	add_action( 'yogastudio_action_before_init_theme', 'yogastudio_template_events_2_theme_setup', 1 );
	function yogastudio_template_events_2_theme_setup() {
		yogastudio_add_template(array(
			'layout' => 'events-2',
			'template' => 'events-2',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 2/', 'yogastudio')
		));
	}
}

// Template output
if ( !function_exists( 'yogastudio_template_events_2_output' ) ) {
	function yogastudio_template_events_2_output($post_options, $post_data) {
		$show_title = true;
		...
		<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
			...
			?><span class="sc_events_item_time"><?php
				echo (trim($start_date[1]) ? $start_date[1] : esc_html__('Whole day', 'yogastudio'))
