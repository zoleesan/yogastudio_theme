<?php
if (yogastudio_get_custom_option('show_slider')=='yes') { 
	...
	if ($slider == 'revo' && function_exists('yogastudio_exists_revslider') && yogastudio_exists_revslider()) {
		...
		if (!empty($slider_alias)) $slider_html = yogastudio_do_shortcode('[rev_slider '.esc_attr($slider_alias).']');
		yogastudio_enqueue_style(  'new-royalslider-core-css', NEW_ROYALSLIDER_PLUGIN_URL . 'lib/royalslider/royalslider.css', array(), null );
		yogastudio_enqueue_script( 'new-royalslider-main-js', NEW_ROYALSLIDER_PLUGIN_URL . 'lib/royalslider/jquery.royalslider.min.js', array('jquery'), NEW_ROYALSLIDER_WP_VERSION, true );
