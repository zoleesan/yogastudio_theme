...
function yogastudio_sc_init(container) {
			...
			.on('click', '.sc_emailer_button', function(e) {
				...
				if (parent.hasClass('sc_emailer_opened')) {
					if (form.length>0 && form.find('input').val()!='') {
						...
						var regexp = new RegExp(YOGASTUDIO_GLOBALS['email_mask']);
						if (!regexp.test(email)) {
							form.find('input').get(0).focus();
							yogastudio_message_warning(YOGASTUDIO_GLOBALS['strings']['email_not_valid']);
						} else {
							jQuery.post(YOGASTUDIO_GLOBALS['ajax_url'], {
								...
								if (rez.error === '') {
									yogastudio_message_info(YOGASTUDIO_GLOBALS['strings']['email_confirm'].replace('%s', email));
									form.find('input').val('');
								} else {
									yogastudio_message_warning(rez.error);
								}
							});
						}
					} else
		...
		container.find('.search_wrap:not(.inited)').each(function() {
			...
			if (jQuery(this).hasClass('search_ajax')) {
				...
				jQuery(this).find('.search_field').keyup(function(e) {
					...
					if (s.length >= YOGASTUDIO_GLOBALS['ajax_search_min_length']) {
						ajax_timer = setTimeout(function() {
							...
							jQuery.post(YOGASTUDIO_GLOBALS['ajax_url'], {
								...
								if (rez.error === '') {
									...
									yogastudio_message_warning(YOGASTUDIO_GLOBALS['strings']['search_error']);
								}
							});
						}, YOGASTUDIO_GLOBALS['ajax_search_delay']);
					}
				});
			}
		});
	...
	if (container.find('.sc_scroll:not(.inited)').length > 0) {
		container.find('.sc_scroll:not(.inited)')
			.each(function () {
				"use strict";
				if (jQuery(this).parents('div:hidden,article:hidden').length > 0) return;
				YOGASTUDIO_GLOBALS['scroll_init_counter'] = 0;
	...
	if (container.find('.sc_slider_swiper:not(.inited)').length > 0) {
		YOGASTUDIO_GLOBALS['swipers_min_width'] = 250;
		container.find('.sc_slider_swiper:not(.inited)')
			.each(function () {
				...
				if (width / spv < YOGASTUDIO_GLOBALS['swipers_min_width']) spv = Math.max(1, Math.floor(width / YOGASTUDIO_GLOBALS['swipers_min_width']));
				var space = jQuery(this).data('slides-space');
				if (space == undefined) space = 0;
				if (YOGASTUDIO_GLOBALS['swipers'] === undefined) YOGASTUDIO_GLOBALS['swipers'] = {};
				YOGASTUDIO_GLOBALS['swipers'][id] = new Swiper('.'+id, {
				...
				if (curSlide > 0)
					YOGASTUDIO_GLOBALS['swipers'][id].slideTo(curSlide-1);
...
function yogastudio_sc_init_scroll_area(obj) {
	...
	if (!yogastudio_check_images_complete(obj) && YOGASTUDIO_GLOBALS['scroll_init_counter']++ < 30) {
...
/*
	...
	if (navi.length > 0) {
		navi.on('click', '.sc_slider_prev,.sc_scroll_prev', function(e) {
			...
			YOGASTUDIO_GLOBALS['swipers'][id].slidePrev();
			e.preventDefault();
			return false;
		})
        .on('click', '.sc_slider_next,.sc_scroll_next', function(e){
			...
			if (swiper.length == 0) swiper = jQuery(this).parents('.sc_slider_controls_wrap,.sc_scroll_controls_wrap').siblings('.swiper-slider-container');
			var id = swiper.attr('id');
			YOGASTUDIO_GLOBALS['swipers'][id].slideNext();
			e.preventDefault();
			return false;
		});
	}
*/
	...
	if (navi.length > 0) {
		navi.on('click', '.post_item', function(e){
			...
			var id = swiper.attr('id');
			YOGASTUDIO_GLOBALS['swipers'][id].slideTo(jQuery(this).index()+1);
			e.preventDefault();
			return false;
		});
	}
...
function yogastudio_sc_sliders_resize() {
	...
	jQuery(slider).each(function() {
		...
		if (last_width==0 || last_width!=width) {
			...
			if (YOGASTUDIO_GLOBALS['swipers'][id].params.slidesPerView != spv) {
				YOGASTUDIO_GLOBALS['swipers'][id].params.slidesPerView = spv;
				YOGASTUDIO_GLOBALS['swipers'][id].params.loopedSlides = spv;
				//YOGASTUDIO_GLOBALS['swipers'][id].reInit();
			}
		}
	...
	container.find('.sc_skills_arc:not(.inited)').each(function () {
		...
		var o = {
			...
			diagram: function(){
				...
				r.circle(c, c, Math.round(w/2)).attr({ stroke: 'none', fill: YOGASTUDIO_GLOBALS['theme_skin_bg_color'] ? YOGASTUDIO_GLOBALS['theme_skin_bg_color'] : '#ffffff' });
				
				var title = r.text(c, c, arc.data('caption')).attr({
					font: Math.round(rad*0.75)+'px "'+YOGASTUDIO_GLOBALS['theme_font']+'"',
					fill: YOGASTUDIO_GLOBALS['theme_skin_color'] ? YOGASTUDIO_GLOBALS['theme_skin_color'] : '#909090'
				}).toFront();
