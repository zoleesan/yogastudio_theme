...
$YOGASTUDIO_GLOBALS['share_links'] = array(
...
if (!function_exists('yogastudio_add_share_link')) {
	function yogastudio_add_share_link($soc, $url='') {
		...
		$YOGASTUDIO_GLOBALS['share_links'] = array_merge( $YOGASTUDIO_GLOBALS['share_links'], $soc );
	}
}
	...
	function yogastudio_show_share_links($args) {
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = empty($soc['url']) && !empty($YOGASTUDIO_GLOBALS['share_links'][$sn]) ? $YOGASTUDIO_GLOBALS['share_links'][$sn] : $soc['url'];
...
if (!function_exists('yogastudio_facebook_og_tags')) {
	function yogastudio_facebook_og_tags() {
		global $post;
		if ( !is_singular() || yogastudio_get_global('blog_streampage')) return;
