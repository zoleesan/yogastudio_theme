...
jQuery(document).ready(function() {
	...
	if (jQuery('#custom_options').length > 0) {
		...
		if (YOGASTUDIO_GLOBALS['demo_time'] > 0) {
			if (yogastudio_get_cookie('yogastudio_custom_options_demo') != 1 ){
				setTimeout(function() { jQuery("#co_toggle").trigger('click'); }, YOGASTUDIO_GLOBALS['demo_time']);
				yogastudio_set_cookie('yogastudio_custom_options_demo', '1', 1);
			}
		}

		yogastudio_custom_options_reset(!YOGASTUDIO_GLOBALS['remember_visitors_settings']);
		...
		jQuery('#custom_options').on('click', '#co_theme_reset', function (e) {
			...
			jQuery('#custom_options .co_section').each(function () {
				...
				jQuery(this).find('div[data-options]').each(function() {
					...
					if (YOGASTUDIO_GLOBALS['remember_visitors_settings']) 
		...
		if (swither.length > 0) {
			...
			jQuery("#custom_options" ).on('click', '.co_switch_box a', function(e) {
				...
				if (YOGASTUDIO_GLOBALS['remember_visitors_settings']) yogastudio_set_cookie(options, value, 1);
		...
		jQuery('#custom_options').on('click', '.iColorPicker', function (e) {
			...
			yogastudio_color_picker_show(null, jQuery(this), function(fld, clr) {
				...
				if (YOGASTUDIO_GLOBALS['remember_visitors_settings']) yogastudio_set_cookie(options, clr, 1);
				if (options == 'bg_color') {
					if (YOGASTUDIO_GLOBALS['remember_visitors_settings'])  {
		...
		jQuery('#custom_options').on('click', '#co_bg_pattern_list a', function(e) {
			...
			if (jQuery("#custom_options .co_switch_box .switcher").data('value') != 'boxed') {
				YOGASTUDIO_GLOBALS['co_add_params'] = {'bg_pattern': val};
		...
		jQuery('#custom_options').on('click', '#co_bg_images_list a', function(e) {
			...
			if (jQuery("#custom_options .co_switch_box .switcher").data('value') != 'boxed') {
				YOGASTUDIO_GLOBALS['co_add_params'] = {'bg_image': val};
				jQuery("#custom_options .co_switch_box a[data-value='boxed']").trigger('click');
			}
			e.preventDefault();
			return false;
