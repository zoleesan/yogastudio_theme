...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		yogastudio_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function yogastudio_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: YOGASTUDIO_GLOBALS['ajax_nonce'],
		action: 'yogastudio_importer_start_import',
	...
	jQuery.post(YOGASTUDIO_GLOBALS['ajax_url'], data, function(response) {
		...
		if (rez.error === '') {
			...
			// Do next action
			if (idx < steps.length) {
				yogastudio_importer_do_action(steps, idx);
			} else {
