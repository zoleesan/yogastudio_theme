...
Version: 0.6.1 (modified by YogaStudio - no backward compatibility!)
...
class WP_Import extends WP_Importer {
	...
	function import( $file ) {
		...
		@set_time_limit(max(30, min(1800, (int) yogastudio_get_theme_option('admin_dummy_timeout'))));
		...
		if ($this->debug) dfl(esc_html__('Process authors', 'yogastudio'));
		...
		try {
			...
			$error = true;
			dfl( sprintf(esc_html__('Error while import: %s', 'yogastudio'), $e->getMessage()) );
	...
	function import_start( $file ) {
		...
		if ($this->debug) dfl(esc_html__('Start import posts', 'yogastudio'));

		if ( ! is_file($file) ) {
			dfl( sprintf(esc_html__('Sorry, there has been an error: %s', 'yogastudio'), esc_html__( 'The file does not exist, please try again.', 'yogastudio')) );
			$this->footer();
			die();
		}
		...
		if ( is_wp_error( $import_data ) ) {
			dfl( sprintf(esc_html__('Sorry, there has been an error: %s', 'yogastudio'), $import_data->get_error_message()) );
			$this->footer();
			die();
		}
	...
	function import_end() {
		...
		if ($this->debug) dfl(esc_html__('Finish import posts', 'yogastudio'));
		...
		if ($this->debug) dfl(esc_html__('All done', 'yogastudio'));
	...
	function get_authors_from_import( $import_data ) {
		...
		if ($this->debug) dfl( esc_html__('Get authors from import', 'yogastudio') );
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					if ($this->debug) dfl( sprintf(esc_html__( 'Failed to import author "%s". Their posts will be attributed to the current user.', 'yogastudio' ), $post['post_author']) );
					continue;
				}
	...
	function get_author_mapping() {
		...
		if ($this->debug) dfl( esc_html__('Author mapping', 'yogastudio') );
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ($this->debug) dfl( sprintf(esc_html__('Author "%s" imported.', 'yogastudio'), $this->authors[$old_login]['author_display_name']) );
				} else {
					if ($this->debug) dfl( sprintf(esc_html__('Failed to create new user for %s. Their posts will be attributed to the current user. Message: %s', 'yogastudio'), $this->authors[$old_login]['author_display_name'], $user_id->get_error_message()) );
				}
			}
	...
	function process_categories() {
		...
		if ($this->debug) dfl( esc_html__('Process categories', 'yogastudio') );
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($cat['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf( esc_html__( 'Category "%s" imported.', 'yogastudio' ).'<br>', esc_html($cat['category_nicename']) ) );
				}
			} else {
				dfl( sprintf( esc_html__( 'Failed to import category "%s"', 'yogastudio' ), esc_html($cat['category_nicename']), $id->get_error_message() ) );
				continue;
			}
		}
	...
	function process_tags() {
		...
		if ($this->debug) dfl( esc_html__('Process tags', 'yogastudio') );
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($tag['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf( esc_html__( 'Tag "%s" imported.', 'yogastudio' ), $tag['tag_name']) );
				}
			} else {
				dfl( sprintf( esc_html__( 'Failed to import post tag "%s". Message: %s', 'yogastudio' ), $tag['tag_name'], $id->get_error_message()) );
				continue;
			}
		}
	...
	function process_terms() {
		...
		if ($this->debug) dfl( esc_html__('Process terms', 'yogastudio') );
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($term['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf(esc_html__( 'Taxonomy "%s": term "%s" imported.', 'yogastudio' ), $term['term_taxonomy'], $term['term_name']) );
				}
			} else {
				dfl( sprintf(esc_html__( 'Failed to import term %s: "%s". Message: %s', 'yogastudio' ), $term['term_taxonomy'], $term['term_name'], $id->get_error_message()) );
				continue;
			}
		}
	...
	function process_posts() {
		...
		if ($this->debug) dfl( sprintf(esc_html__('Process posts %s', 'yogastudio'), $this->start_from_id > 0 
																						? sprintf(esc_html__('(continue from ID = %s)', 'yogastudio'), $this->start_from_id) 
																						: ''
											)
								);
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				...
				dfl( sprintf( esc_html__( 'Failed to import post "%s": Invalid post type "%s"', 'yogastudio' ), $post['post_title'], $post['post_type']) );
				...
				do {	
					...
					if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
						dfl( sprintf(esc_html__('%s "%s" already exists.', 'yogastudio'), $post_type_object->labels->singular_name, $post['post_title']) );
						...
						if ( is_wp_error( $post_id ) ) {
							dfl( sprintf(esc_html__( 'Failed to import %s: "%s". Message: %s', 'yogastudio' ), $post_type_object->labels->singular_name, $post['post_title'], $post_id->get_error_message()) );
							break;
						}
						...
						if ( $this->debug ) dfl( sprintf( esc_html__( '%s "%s" (ID=%s) imported.', 'yogastudio' ), $post_type_object->labels->singular_name, $post['post_title'], $post_id) );
					}
						...
						foreach ( $post['terms'] as $term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									dfl( sprintf( esc_html__( 'Failed to import post term %s: "%s". Message: %s', 'yogastudio' ), $taxonomy, $term['name'], $t->get_error_message()) );
						...
						foreach ( $newcomments as $key => $comment ) {
							...
							if ( ! $post_exists || ! comment_exists( $comment['comment_author'], $comment['comment_date'] ) ) {
								...
								foreach( $comment['commentmeta'] as $meta ) {
									$value = yogastudio_unserialize( $meta['value'] );
							...
							if ( $key ) {
								...
								if ( ! $value ) {
									...
									if (is_serialized($value)) {
										$value = yogastudio_unserialize( $value );
										if ( $value===false ) {
											if ($this->debug) dfl( sprintf(esc_html__('Post (ID=%s) - error unserialize postmeta: %s', 'yogastudio'), $post['post_id'], $key) );
											$value = $meta['value'];
											$replace = false;
										}
									}
			...
			$admin_tm = max(0, min(1800, (int) yogastudio_get_theme_option('admin_dummy_timeout')));
			...
			yogastudio_fpc($this->import_log, trim(max($original_post_ID, $this->start_from_id)) . '|' . trim($result));
			if ($this->debug) dfl( sprintf( esc_html__('Post (ID=%s) imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'yogastudio'), $post['post_id'], $result.'%', $this->max_time, time() - $this->start_time) );
	...
	function process_menu_item( $item ) {
		...
		if ($this->debug) dfl( esc_html__('Process menu item', 'yogastudio') );
		...
		if ( ! $menu_slug ) {
			if ($this->debug) dfl( sprintf(esc_html__( 'Menu item "%s" (id=%s) skipped due to missing menu slug', 'yogastudio' ), $item['post_title'], $item['post_id']) );
			return;
		}

		$menu_id = term_exists( $menu_slug, 'nav_menu' );
		if ( ! $menu_id ) {
			if ($this->debug) dfl( sprintf( esc_html__( 'Menu item "%s" (id=%s) skipped due to invalid menu slug: %s', 'yogastudio' ), $item['post_title'], $item['post_id'], $menu_slug) );
			return;
		} else {
		...
		$_menu_item_classes = yogastudio_unserialize( $_menu_item_classes );
		...
		if ($_menu_item_type == 'custom' && strpos($_menu_item_url, $this->demo_url) !== false) {
			$_menu_item_url = str_replace($this->demo_url, get_site_url() . (yogastudio_substr($this->demo_url, -1)=='/' ? '/' : ''), $_menu_item_url);
		}
		...
		if ( $id && ! is_wp_error( $id ) ) {
			if ($this->overwrite) {
				...
				if ( ! empty( $item['postmeta'] ) ) {
					foreach ( $item['postmeta'] as $meta ) {
						...
						if (yogastudio_substr($key, 0, 11)=='_menu_item_') continue;
						...
						if ( $key ) {
							...
							if ( ! $value ) {
								...
								if (is_serialized($value)) {
									...
									if ( $value === false ) {
										if ($this->debug) {
											dfl(sprintf( esc_html__('Menu item (ID=%s) - error unserialize postmeta: %s=', 'yogastudio'), $item['post_id'], $key) );
										}
										$value = $meta['value'];
										$replace = false;
									}
			...
			if ( $this->debug ) dfl( sprintf( esc_html__( 'Menu item "%s" (ID=%s) imported.', 'yogastudio'), $item['post_title'], $item['post_id']) );
		}
	}
	...
	function process_attachment( $post, $url ) {
		...
		if ($this->debug) dfl( esc_html__('Process attachment', 'yogastudio') );
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			$msg = esc_html__('Invalid file type', 'yogastudio');
			dfl( $msg );
...
function fetch_remote_file( $url, $post ) {
	...
	function replace_uploads($str) {
		return yogastudio_replace_uploads_url($str, $this->uploads_folder);
	}
	...
	function backfill_parents() {
		...
		if ($this->debug) dfl(esc_html__('Backfill parents', 'yogastudio'));
	...
	function backfill_attachment_urls() {
		...
		if ($this->debug) dfl(esc_html__('Backfill attachments', 'yogastudio'));
	...
	function remap_featured_images() {
		...
		if ($this->debug) dfl(esc_html__('Remap featured images', 'yogastudio'));
	 ...
	 */
	function parse( $file ) {
		if ($this->debug) dfl( sprintf(esc_html__('Parse file %s', 'yogastudio'), $file) );
